"""
GMC 5-Minute Locked Strategy — Backtest Engine
================================================
Bar-by-bar simulation with:
  - 1-contract LONG-only trading
  - ATR-based SL/TP
  - Optional trailing stop (ratchet per bar)
  - Full metrics: win rate, ROI, Sharpe, max drawdown, profit factor
  - Equity curve

Contract spec (Micro Gold MGC):
  - Contract size : 10 oz
  - Tick size     : $0.10 (= $1 per contract)
  - Commission    : $0.62 round-trip
"""
...
from .strategy_5min_locked import LockedStrategy5Min, DEFAULT_LOCKED_PARAMS
...
@dataclass
class LockedBacktestResult:
    trades:           list[LockedTrade] = field(default_factory=list)
...
class BacktesterLocked5Min:
    ...
    ) -> LockedBacktestResult:
        full_params = {**DEFAULT_LOCKED_PARAMS, **(params or {})}
        strategy    = LockedStrategy5Min(full_params)
    ...
    def _simulate(
        ...
        df: pd.DataFrame,
        signals: pd.Series,
        strategy: LockedStrategy5Min,
        params: dict,
    ) -> tuple[list[LockedTrade], list[float]]:
        """Bar-by-bar simulation — numpy arrays for hot loop speed."""
        ...
        for i in range(1, len(df)):
            ...
            if position is not None:
                ...
                elif use_trailing and lows[i] <= position["sl"]:
            ...
            if sig_arr[i] == 1:
                ...
                logger.debug("ENTRY %s @ %.2f  SL=%.2f  TP=%.2f", times[i], entry_price, sl, tp)
    ...
    def _compute_metrics(
        ...
        # Sharpe ratio (annualised, assume 5m bars → ~252×78 bars/year)
