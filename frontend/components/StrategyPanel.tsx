...
export default function StrategyPanel({ symbol, period }: Props) {
    ...
    <div className="rounded-xl border border-slate-800 bg-gradient-to-br from-slate-900/90 to-slate-900/60 overflow-hidden">
      ...
      <div className="flex items-center gap-3 border-b border-slate-800/60 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="mt-0.5 text-[9px] text-slate-500 truncate">EMA + RSI + Supertrend + Vol · Grid Search · {symbol}</p>
        </div>
        <button
          onClick={handleRun}
          disabled={loading}
              ...
              <table className="w-full text-[10px]">
                ...
                <tbody>
                  {result.trades.map((t, i) => {
                    ...
                    return (
                      <tr key={i} className={`border-b border-slate-800/30 hover:bg-slate-800/20 ${win ? "" : "bg-rose-950/10"}`}>
