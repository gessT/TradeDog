...
import PositionCard from "./trader/PositionCard";
    ...
    <div className={`h-full flex flex-col backdrop-blur-sm relative ${
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        ...
        {tab === "paper" && (() => {
          ...
          // 3-day metrics (last 3 calendar days: today, yesterday, day before)
          const threeDayCutoff = new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
          const threeDayTrades = closedTrades.filter((t) => (t.exit_time ?? t.entry_time) >= threeDayCutoff);
          const threeDayPnl = threeDayTrades.reduce((s, t) => s + t.pnl, 0);
          const threeDayWins = threeDayTrades.filter((t) => t.pnl > 0).length;
          const threeDayLosses = threeDayTrades.filter((t) => t.pnl < 0).length;
                ...
                <div className="rounded-xl ring-1 ring-white/[0.08] bg-slate-900/40 flex flex-col overflow-hidden">
                  ...
                  <div className="flex-1 grid grid-cols-2 gap-px bg-white/[0.04]">
                    {[
                      ...
                      { label: "3 Days", value: `${threeDayPnl >= 0 ? "+" : ""}$${threeDayPnl.toFixed(2)}`, color: threeDayPnl >= 0 ? "text-emerald-400" : "text-red-400" },
                      { label: "Win Rate", value: `${wr}%`, color: wr >= 50 ? "text-emerald-400" : "text-amber-400" },
                      { label: "3D W/L", value: `${threeDayWins}/${threeDayLosses}`, color: threeDayWins > threeDayLosses ? "text-emerald-400" : threeDayWins < threeDayLosses ? "text-red-400" : "text-white/50" },
                    ].map((s, i) => (
              ...
              <div className="px-3 pb-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {closedTrades.length > 0 && (
                    <>
                      <span className="text-[8px] text-white/30">All:</span>
                      ...
                      {threeDayTrades.length > 0 && (
                        <>
                          ...
                          <span className="text-[8px] text-white/30">3D:</span>
                          <span className="text-[8px] text-emerald-400/50">{threeDayWins}W</span>
                          <span className="text-[8px] text-red-400/50">{threeDayLosses}L</span>
                          <span className={`text-[8px] font-black tabular-nums ml-0.5 ${threeDayPnl >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                            {threeDayPnl >= 0 ? "+" : ""}${threeDayPnl.toFixed(2)}
                          </span>
                        </>
                      )}
                    </>
