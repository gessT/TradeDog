...
export default function USDashboard() {
  const { symbol, setSymbol, period, setPeriod, stockName, rows, rawPoints, loading, error, refresh, lastRefreshed } = useStock("AAPL");
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="hidden md:flex md:w-1/3 flex-shrink-0 flex-col overflow-y-auto border-r border-slate-800/60 bg-slate-900/40">
          <div className="flex-1 overflow-y-auto p-3 space-y-3">
            <DailyScanner onSelectSymbol={setSymbol} market="US" />
            <div className="rounded-lg border border-slate-800/60 bg-slate-900/50 p-2.5">
              <NearATH onSelectSymbol={setSymbol} market="US" />
            </div>
            <div className="rounded-lg border border-slate-800/60 bg-slate-900/50 p-2.5">
              <TopVolume onSelectSymbol={setSymbol} market="US" />
            </div>
            <div className="rounded-lg border border-slate-800/60 bg-slate-900/50 p-2.5">
              <SectorList onSelectSymbol={setSymbol} onSelectSector={handleSelectSector} market="US" />
            </div>
          </div>
        </aside>

        {/* ── RIGHT 2/3: Chart + Strategy ─────────── */}
        <section className="w-full md:w-2/3 flex flex-col overflow-hidden">
          ...
          <div className="flex flex-col" style={{ height: "50%" }}>
            <div className="flex items-center gap-2 px-4 py-2 border-b border-slate-800/60">
              {sectorChartData ? (
                <>
                  <span className="text-sm font-bold text-blue-300">{sectorChartName}</span>
          ...
          {/* Strategy panels (bottom half) — V2 only for US */}
