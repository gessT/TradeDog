...
export default function USTopBar({
  ...
  return (
    <div className="shrink-0 flex items-center gap-3 px-3 py-1.5 border-b border-slate-800/60 bg-slate-950/90 backdrop-blur-sm">
      {/* ── Symbol Selector ─────────────────────────── */}
      <div className="relative">
        <button
          onClick={() => setSearchOpen(!searchOpen)}
          className="flex items-center gap-2 px-2.5 py-1 rounded-lg border border-slate-700/60 bg-slate-900/80 hover:bg-slate-800/80 transition group"
        >
          <span className="text-sm font-bold text-blue-300">{symbol}</span>
          <span className="text-[9px] text-slate-500 max-w-[80px] truncate">{symbolName}</span>
          ...
          <div className="absolute top-full left-0 mt-1 w-56 bg-slate-900 border border-slate-700/80 rounded-lg shadow-2xl shadow-black/40 z-50 overflow-hidden">
            <div className="p-2 border-b border-slate-800/60">
              <input
                ...
                className="w-full px-2 py-1 text-xs bg-slate-800 border border-slate-700 rounded text-slate-200 placeholder-slate-600 outline-none focus:border-blue-500/60"
              />
            </div>
            <div className="max-h-48 overflow-y-auto">
              {filtered.map((s) => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-1.5 text-left hover:bg-slate-800/60 transition text-xs ${
      ...
      {/* ── Price & Market Data ─────────────────────── */}
      <div className="flex items-center gap-3 border-l border-slate-800/40 pl-3">
        <div className="flex flex-col">
          <span className={`text-base font-bold tabular-nums ${up ? "text-emerald-400" : "text-rose-400"}`}>
            ${price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </span>
          <span className={`text-[9px] font-medium tabular-nums ${up ? "text-emerald-400/80" : "text-rose-400/80"}`}>
            {up ? "+" : ""}{change.toFixed(2)} ({up ? "+" : ""}{changePct.toFixed(2)}%)
          </span>
        </div>
        <div className="flex flex-col gap-0.5 text-[8px] tabular-nums">
      ...
      {/* ── Strategy Selector ──────────────────────── */}
      <div className="flex items-center gap-1.5 border-l border-slate-800/40 pl-3">
        <span className="text-[8px] text-slate-600 uppercase tracking-wider">Strategy</span>
        <select
          value={strategy}
          onChange={(e) => onStrategyChange(e.target.value)}
          className="text-[10px] px-2 py-0.5 rounded border border-slate-700 bg-slate-800 text-slate-200 outline-none cursor-pointer hover:border-blue-500/50 transition"
        >
          {STRATEGIES.map((s) => (
        ...
        {MODES.map((m) => {
          ...
          return (
            <button
              ...
              className={`px-2.5 py-0.5 text-[9px] font-bold tracking-wide transition ${
      ...
      <button
        ...
        className={`flex items-center gap-1.5 px-3 py-1 rounded-lg border text-[10px] font-bold tracking-wide transition ${
          tradingActive
      ...
      >
        ...
        {tradingActive ? "TRADING ON" : "TRADING OFF"}
      </button>
    </div>
  );
}
