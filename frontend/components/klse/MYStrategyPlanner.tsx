...
import { MY_DEFAULT_SYMBOLS, MY_SECTORS, MY_STOCKS_BY_SECTOR } from "../../constants/myStocks";
...
export default function MYStrategyPlanner({ activePreset, onApply, onPresetsChanged, onTagSaved, favSymbols = [], allTags = [], selectedSymbol }: Props) {
  ...
  const toggleFavorite = useCallback(async (id: number) => {
    await fetch(`${API_BASE}/stock/my-strategy-presets/${id}/favorite`, { method: "PUT" });
    await fetchPresets();
  }, [fetchPresets]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      await fetch(`${API_BASE}/stock/my-strategy-presets`, {
        method: "POST",
  ...
  const handleDelete = async (id: number, name: string, strategyType: string) => {
    ...
    try {
      await fetch(`${API_BASE}/stock/my-strategy-presets/${id}`, { method: "DELETE" });
      await fetchPresets();
  ...
  const handleForceDelete = async () => {
    ...
    try {
      ...
      await fetch(`${API_BASE}/stock/my-strategy-presets/${deleteDialog.id}`, { method: "DELETE" });
      await fetchPresets();
      onTagSaved?.();
  ...
  const runCompare = async () => {
    ...
    const symbols = compareSector === "FAVS"
      ? (favSymbols.length > 0 ? [...favSymbols] : [...MY_DEFAULT_SYMBOLS])
      : (MY_STOCKS_BY_SECTOR[compareSector] ?? []).map((s) => s.symbol);
                      ...
                      return (
                        <span
                          onClick={async (e) => {
                            ...
                            try {
                              if (existingTag) {
                                await fetch(`${API_BASE}/stock/my-stock-tags/${existingTag.id}`, { method: "DELETE" });
                                onTagSaved?.();
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <optgroup label="── By Sector ──">
                {MY_SECTORS.map((s) => (
                  <option key={s} value={s}>{s} ({(MY_STOCKS_BY_SECTOR[s] ?? []).length})</option>
                ))}
              </optgroup>
            </select>
            <button
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className="text-[11px] text-slate-500">RM{editing.capital.toLocaleString()}</span>
                      <span className="text-[11px] text-slate-600">·</span>
