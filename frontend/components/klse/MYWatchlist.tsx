...
export default function MYWatchlist({ region = "MY", activeSymbol, onSelectSymbol, stockTags = [], favSymbols, onToggleFav, colorLabels = [], onSetColor, onRemoveColor }: Props) {
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden bg-slate-950/60">
      {/* ═══ SCANNER ═══ */}
      <div className="border-b border-slate-700/50 shrink-0 bg-gradient-to-r from-cyan-500/8 via-blue-500/6 to-violet-500/8">
        <button
          onClick={() => setScannerOpen((p) => !p)}
          className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-slate-800/30 transition"
        >
          <span className="inline-flex items-center justify-center w-5 h-5 rounded-md border border-cyan-500/40 bg-cyan-500/15 text-[11px] text-cyan-300 shrink-0">
            🛰️
          </span>
          ...
          <svg className={`w-3.5 h-3.5 text-cyan-300/70 shrink-0 transition-transform ${scannerOpen ? "rotate-90" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {scannerOpen && (
          <div className="px-2.5 pb-2.5 space-y-1.5">
            <button
              ...
              className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-left transition bg-gradient-to-r from-amber-500/14 to-orange-500/8 border border-amber-500/30 hover:from-amber-500/22 hover:to-orange-500/14"
            >
              <span className="text-base shrink-0">🏔️</span>
              <div className="min-w-0 flex-1">
                <div className="text-[10px] font-bold text-slate-100">Near ATH</div>
                <div className="text-[8px] text-amber-300/70">Stocks closest to All-Time High</div>
              </div>
              {athScanning ? (
                <svg className="w-4 h-4 text-amber-300 animate-spin shrink-0" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
              ) : (
                <svg className="w-4 h-4 text-amber-200/70 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.3} d="M9 5l7 7-7 7" /></svg>
              )}
            </button>
            ...
            <button
              ...
              disabled={vbScanning}
              className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-left transition bg-gradient-to-r from-violet-500/14 to-fuchsia-500/8 border border-violet-500/30 hover:from-violet-500/22 hover:to-fuchsia-500/14"
            >
              <span className="text-base shrink-0">📊</span>
              <div className="min-w-0 flex-1">
                <div className="text-[10px] font-bold text-slate-100">Vol Breakout</div>
                <div className="text-[8px] text-violet-300/70">Big volume &rarr; breakout / range / breakdown</div>
              </div>
              {vbScanning ? (
                <svg className="w-4 h-4 text-violet-300 animate-spin shrink-0" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" /></svg>
              ) : (
                <svg className="w-4 h-4 text-violet-200/70 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.3} d="M9 5l7 7-7 7" /></svg>
              )}
            </button>
            ...
            <div className="flex items-center gap-1">
              <div className="relative flex-1">
                <button
                  ...
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-left transition bg-gradient-to-r from-cyan-500/14 to-blue-500/10 border border-cyan-500/35 hover:from-cyan-500/22 hover:to-blue-500/14"
                >
                  <span className="text-base shrink-0">{STRATEGY_OPTIONS.find(s => s.key === oppStrategy)?.icon ?? "🎯"}</span>
                  <div className="min-w-0 flex-1">
                    <div className="text-[10px] font-bold text-slate-100">Buy Opportunity</div>
                    <div className="text-[8px] text-cyan-300/75">{STRATEGY_OPTIONS.find(s => s.key === oppStrategy)?.label ?? "Select"} — scan active signals</div>
                  </div>
                  <svg className={`w-3.5 h-3.5 text-cyan-300/70 transition-transform ${oppStrategyDropdown ? "rotate-180" : ""}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <button
                ...
                className="shrink-0 px-3 py-2.5 rounded-lg bg-cyan-500/25 border border-cyan-400/40 hover:bg-cyan-500/35 text-cyan-200 text-[10px] font-black tracking-wide transition disabled:opacity-50"
              >
                {oppScanning ? (
